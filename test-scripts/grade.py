...
# Path to Batch Files
PBF = '/stage/classes/archive/2022/winter/15400-1/proj/p4/batch-files'
PEF = '/stage/classes/archive/2022/winter/15400-1/proj/p4/expected-output'
...
batchfiles = OrderedDict((('gooduser_basic', 7),
                          ('gooduser_args', 7),
                          ('gooduser_redirection', 2), #error
                          ('gooduser_multipleCommand', 7), #extra new lines
                          ('buildin_wrongFormat', 7), #error
                          ('cd_toFolderNotExist', 3), 
                          ('badCommand', 4),
                          ('complexCommand', 6), #extra new lines
                          ('multipleCommand', 8), #hits an exit and stops i think
                          ('complexRedirection_format', 2), #no
                          ('complexRedirection_illegal', 1), #same as above me tinks
			              ('emptyInput', 1),
                          ('1KCommand', 3)))
...
def safeExec(cli):
    ...
    try:
        ...
        while (isProcessExist('myshell')):
            system('killall -u {} -r myshell'.format(usrname))
        return (True, ('', ''))
    t.join()
    return (False, output)
...
def runAll():
    ...
    totalScore += score
    scorePrint('total', totalScore, 63)
